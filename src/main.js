...
function setupNavigation() {
    ...
    btnHome.onclick = () => {
        const homeSceneId = appData.start_scene || 'Front_00.jpg';
	...
	window.addEventListener('keydown', (e) => {
        ...
        if (e.key.toLowerCase() === 'home') {
            const homeSceneId = appData.scenes['Front_00.jpg'] ? 'Front_00.jpg' : 
                               (appData.scenes['Front_00'] ? 'Front_00' : appData.start_scene);
...
function loadScene(sceneId, isBack = false) {
    ...
    if (!scene) {
        if (sceneId.match(/\.(jpg|jpeg|png|webp|dat)$/i)) {
